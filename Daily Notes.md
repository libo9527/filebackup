...
https://juejin.im/post/5e8fcf7a6fb9a03c3a08852a





https://stackoverflow.com/questions/2399753/select-from-table-by-knowing-only-date-without-time-oracle/2399800





在 Java 中，创建子类对象时，因为可能会用到父类的成员变量或者成员方法，所以会先调用父类的初始化方法(包括了构造方法)，然后逐级向上调用，直到 Object 类的无参构造方法。也就是说，在编译的时候，子类的构造方法默认加了`super();`语句，除非在构造方法体中调用了类中其他的构造方法或者显式的通过`super(...)`调用了父类的有参构造方法，所以当一个类所继承的类没有无参构造方法的时候，必须在子类的构造方法中显式的用 super 来调用父类的构造方法。但是，**调用了构造方法并不一定会生成对象**，所以在调用子类的构造方法时，并不会产生父类的对象。



Java 在其它包中无法引用缺省包(src)中的类。因为缺省包没有包名，在其他包下没有包名是指在本包中，如果一定要使用可以使用反射机制引用。
